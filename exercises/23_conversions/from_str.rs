...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if person.len() != 2 { Err(ParsePersonError::BadLen) }
        else if person[0].is_empty() { Err(ParsePersonError::NoName) }
        else {
            let age = person[1].parse::<u8>();
            match age {
                Err(age) => Err(ParsePersonError::ParseInt(age)),
