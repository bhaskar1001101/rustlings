...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if tuple < (0, 0, 0) || tuple > (255, 255, 255) { Err(Self::Error::IntConversion) }
        else { Ok(Color{red: tuple.0 as u8, green: tuple.1 as u8, blue: tuple.2 as u8}) }
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if arr < [0, 0, 0] || arr > [255, 255, 255] { Err(Self::Error::IntConversion) }
        else { Ok(Color { red: arr[0] as u8, green: arr[1] as u8, blue: arr[2] as u8 }) }
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 { Err(Self::Error::BadLen) }
        else if slice[0] < 0 || slice[1] < 0 || slice[2] < 0 { Err(Self::Error::IntConversion) }
        else if slice[0] > 255 || slice[1] > 255 || slice[2] > 255 { Err(Self::Error::IntConversion) }
        else { Ok(Color { red: slice[0] as u8, green: slice[1] as u8, blue: slice[2] as u8 }) }
    }
}
